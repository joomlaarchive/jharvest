...
;(function($){
	"use strict";
	$.metadata = function(container, options){
		this.$container = $(container);

		// check if alredy exist
		if(this.$container.data("metadata")){
			return self;
		}

		// Add a reverse reference to the DOM object
		this.$container.data("metadata", self);

		// merge options
		this.options = $.extend({}, $.metadata.defaults, options);
		...
		this.$container.trigger('metadata-ready');
	};

	// prepare a template that we will use repeating
	$.metadata.prototype.prepareTemplate = function(){
	...
	// add new row
	$.metadata.prototype.addRow = function(after){
		...
		this.$container.trigger('metadata-row-add', $row);
		return $row;
	};

	// remove row
	$.metadata.prototype.removeRow = function($row){
		...
		this.$container.trigger('metadata-row-remove', $row);
		$row.remove();
	};

	// fix names ind id`s for field that in $row
	$.metadata.prototype.fixUniqueAttributes = function($row, count){
		this.lastRowNum++;
	...
	$.metadata.prototype.clearScripts = function($row){
	...
	$.metadata.prototype.fixScripts = function($row){
		//color picker
	...
	$.metadata.defaults = {
        ...
        repeatableElement: ".metadata-group",
		rowTemplateSelector: 'script.metadata-template-section', // selector for the row template <script>
	...
	$.fn.metadata = function(options){
		return this.each(function(){
			...
			var inst = new $.metadata(this, options);
			$(this).data('metadata', inst);
		});
	};
